...
names(gdp)[names(gdp) ==
  "GDP.per.capita..constant.2017.international."]<-"gdp_pc"
```
...
air$outdoor<- air$pm + air$ozone


air_c1 <- air[air$Entity == country1, ]
air_c2 <- air[air$Entity == country2, ]


ymax <- max(c(air_c1$indoor, air_c1$outdoor,
              air_c2$indoor, air_c2$outdoor), na.rm = TRUE)


plot(air_c1$Year, air_c1$indoor,
     type = "l",
     col = "red",
     lty = 1,
     ylim = c(0, ymax),
     ...
     main = "Indoor and Outdoor Air Pollution Death Rates",
     las = 1)

lines(air_c1$Year, air_c1$outdoor,
      col = "red",
      lty = 2)


lines(air_c2$Year, air_c2$indoor,
      col = "blue",
      lty = 1)
lines(air_c2$Year, air_c2$outdoor,
      col = "blue",
      lty = 2)

legend("topright",
       legend = c("China – Indoor", "China – Outdoor",
                  "United States – Indoor", "United States – Outdoor"),
...
gdp_colname <- grep("GDP", colnames(merged), value = TRUE)[1]
gdp_num <- as.numeric(as.character(merged[[gdp_colname]]))
log_gdp <- log(gdp_num)
outdoor <- merged$pm + merged$ozone

ok_in  <- is.finite(log_gdp) & is.finite(merged$indoor)
ok_out <- is.finite(log_gdp) & is.finite(outdoor)

ymax <- max(c(merged$indoor[ok_in], outdoor[ok_out]), na.rm = TRUE)
par(mfrow = c(1, 2))
plot(log_gdp[ok_in], merged$indoor[ok_in],
     pch = 20,
     col = "gray40",
     xlab = "Log GDP per Capita",
     ylab = "Indoor Air Pollution Death Rate",
     main = "Indoor Air Pollution vs GDP",
     ylim = c(0, ymax),
     las = 1)
plot(log_gdp[ok_out], outdoor[ok_out],
     pch = 20,
     col = "gray40",
     xlab = "Log GDP per Capita",
     ylab = "Outdoor Air Pollution Death Rate",
     main = "Outdoor Air Pollution vs GDP",
     ylim = c(0, ymax),
     las = 1)
par(mfrow = c(1, 1))
```
